...
def check_clinvar_file(app: Context, build: GenomeBuild):
    downloaded_files = list(app.config.pathogenic_root.glob("{build}/clinvar*.vcf.gz"))
    label = "ClinVar"
...
def check_clingen_hits_file(app: Context, build: GenomeBuild):
    gene_file = app.config.pathogenic_root / f"{build}/ClinGen_gene_curation_list_{build}.tsv"
    region_file = app.config.pathogenic_root / f"{build}/ClinGen_region_curation_list_{build}.tsv"
    label = "ClinGen HITS"
...
def check_dbvar_file(app: Context, build: GenomeBuild):
    del_file = (
        app.config.pathogenic_root / f"{build}/{build}.nr_deletions.pathogenic.tsv.gz"
    )  # Loss
    dup_file = (
        app.config.pathogenic_root / f"{build}/{build}.nr_duplications.pathogenic.tsv.gz"
    )  # Gain
    ins_file = (
        app.config.pathogenic_root / f"{build}/{build}.nr_insertions.pathogenic.tsv.gz"
    )  # Ins
    label = "dbVar"
...
def check_omim_file(app: Context, build: GenomeBuild):
    downloaded_files = list(app.config.pathogenic_root.glob(f"{build}/*_morbid.tsv.gz"))
    label = "OMIM"
...
def _pathogenic_files(app: Context, build: GenomeBuild, tmp: bool = False):
    ...
    for sv_type in PATHOGENIC_SVTYPES:
        files.append(
            app.config.pathogenic_root / f"{build}/pathogenic_{sv_type}_SV_{build}.{ftype}.bed"
        )
    return files
