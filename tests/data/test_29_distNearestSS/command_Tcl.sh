...
# BND left in the last exon of a gene + BND right outside the gene
##################################################################

# Input SV: chr8:7366741-7368724

# gr "NM_001037668" $ANNOTSV/share/AnnotSV/Annotations_Human/Genes/GRCh37/genes.RefSeq.sorted.bed
# 8       7353367 7366833 +       DEFB107A        NM_001037668    7353455 7366745 7353367,7366602,        7353525,7366833,
# => NM_001037668 : strand = +

# The "SVleft" (7366741) is located in the last exon of the transcript NM_001037668 (7366602-7366833)
# The BND is 139 bp after the start of the last exon (3') : 7366602 + 139 = 7366741
#         and 92 bp before the end of the exon (last exon => the end of the exon does not correspond to a splice site)
# => we can only calculate the distance in relation to the site in 3' (acceptor)
# => distNearestSS = 139 in 3'
...
# 8_7366741_7368724_DEL_1 full    DEFB107A;DEFB107B
# 8_7366741_7368724_DEL_1 split   DEFB107A        NM_001037668    exon2-txEnd     CDS-3'UTR       138     3'
# 8_7366741_7368724_DEL_1 split   DEFB107B        NM_001040705    exon2-txEnd     CDS-3'UTR       138     3'

annotations=`$cut "./output/test1_tcl.annotated.tsv" "Tx;Dist_nearest_SS;Nearest_SS_type" | grep "NM_001037668"`
if [ `echo $annotations | grep -c "NM_001037668 138 3'"` == 1 ]
then
        echo "Ok"
else
...
# grep "bbs10" $ANNOTSV/share/AnnotSV/Annotations_Human/Genes/GRCh37/genes.RefSeq.sorted.bed
...
# grep "bbs5" $ANNOTSV/share/AnnotSV/Annotations_Human/Genes/GRCh37/genes.RefSeq.sorted.bed
...
# Input SV: 11:7716903-7717219 DEL

# grep "NM_198185" $ANNOTSV/share/AnnotSV/Annotations_Human/Genes/GRCh37/genes.RefSeq.sorted.bed
# 11      7711153 7728024 -       OVCH2   NM_198185       7711185 7727941 7711153,7712499,7713132,7716288,7716800,7717219,7718011,7718255,7720296,7721842,7722870,7723262,7723703,7725244,7726111,7727853,        7711244,7712631,7713226,7716419,7716903,7717257,7718136,7718346,7720320,7722032,7723022,7723358,7723876,7725336,7726221,7728024,
...
# 11_7716903_7717219_DEL_1        split   OVCH2   NM_198185       intron11-intron11       CDS     0       3'
annotations=`$cut "./output/test6_tcl.annotated.tsv" "Tx;Location;Location2;Dist_nearest_SS;Nearest_SS_type" | grep "NM_198185"`
if [ `echo $annotations | grep -c "NM_198185 intron11-intron11 CDS 0 3'"` == 1 ]
then
        echo "Ok"
else
